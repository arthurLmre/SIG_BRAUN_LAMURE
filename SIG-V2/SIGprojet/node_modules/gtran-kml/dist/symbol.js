...
function addFeatureSymbol(kml, geomType, symbols) {
  kml.findall(".//Placemark").forEach(function (place) {
    var styleId = place.findtext("./ExtendedData/Data[@name=\"" + config.DEFAULT_STYLE_ID + "\"]/value");
...
function addSymbol(kml, geomType, symbol) {
  var id = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "kml_symbol";
...
function toKmlColor(symbol) {
  switch (_typeof(symbol.color)) {
    case "string":
      ...
      var color = symbol.color.substr(5, 2) + symbol.color.substr(3, 2) + symbol.color.substr(1, 2);
      break;
    case "object":
      var color = getFullHexagonValue(symbol.color[2]) + getFullHexagonValue(symbol.color[1]) + getFullHexagonValue(symbol.color[0]);
